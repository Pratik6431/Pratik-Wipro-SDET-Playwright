...
function applyPromo(cart, promoCallback) {
  ...
  if (!cart || !Array.isArray(cart)) return Promise.resolve([]);
  ...
  const updatedCart = cart.map(item => {
    ...
    const isEligibleForDiscount = promoCallback(item);

    if (isEligibleForDiscount) {
      const discountedPrice = originalPrice * 0.9; // 10% reduction
...
applyPromo(cart, isEligible).then(updatedData => {
