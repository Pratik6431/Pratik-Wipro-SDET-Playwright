...
You are given a list of raw flight strings from a travel agency's database. The data 
is unformatted and needs to be standardized into objects for a front-end display.
...
**Requirements:**
 ...
 4. **Filtering:** Create a new array containing only flights where the price 
      is between **$100 and $500** (inclusive).
...
function cleanFlightData(flights) {
  const processedFlights = flights
    .map(flightStr => {
      ...
      const [route, priceRaw] = flightStr.split(':');
  ...
  return JSON.stringify(processedFlights);
}
